...
bool BarnacleFlashPages(void* dest, void* src, uint32_t size)
{
    ...
    for(uint32_t n = 0; n < ((size + sizeof(uint64_t) - 1) / sizeof(uint64_t)); n++)
    {
        ...
        if(result == false)
        {
            logError("HAL_FLASH_Program() failed.\r\n");
            goto Cleanup;
        }
    }
...
bool BarnacleWriteLockLoader()
{
    ...
    if(ob.RDPLevel == OB_RDP_LEVEL_0)
    {
        ...
        {
            result = false;
            logError("HAL_FLASH_OB_Unlock() failed.\r\n");
            goto Cleanup;
        }
        ...
        {
            result = false;
            logError("HAL_FLASHEx_OBProgram() failed.\r\n");
            goto Cleanup;
        }
        logInfo("Option bytes written, generating System Reset. Power cycle device to load the new option byte values.\r\n");
        HAL_FLASH_OB_Launch();
#else
    ...
    {
        result = false;
        logError("Undefined OB_RDP_LEVEL\r\n");
    }
...
bool BarnacleInitialProvision()
{
    ...
    dbgPrintAppend("\r\n");
    logInfo("DeviceCert is %s and %s\r\n", ((IssuedCerts.info.flags & BARNACLE_ISSUEDFLAG_PROVISIONIED) ? "ISSUED" : "SELFSIGNED"),
                                           ((IssuedCerts.info.flags & BARNACLE_ISSUEDFLAG_WRITELOCK) ? "WRITELOCKED" : "WRITEABLE"));
//    logInfo("%s", (char*)&IssuedCerts.certBag[IssuedCerts.info.certTable[BARNACLE_ISSUED_DEVICE].start]);
//    if(IssuedCerts.info.certTable[BARNACLE_ISSUED_ROOT].size != 0)
//    {
//        logInfo("%s", (char*)&IssuedCerts.certBag[IssuedCerts.info.certTable[BARNACLE_ISSUED_ROOT].start]);
//    }
    logInfo("CodeAuthority is %s", ((IssuedCerts.info.flags & BARNACLE_ISSUEDFLAG_AUTHENTICATED_BOOT) ? "LOCKED to\r\n0x" : "UNLOCKED\r\n"));
