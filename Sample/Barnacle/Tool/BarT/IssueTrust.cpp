...
)
{
    ...
    try
    {
        ...
        std::wstring certFileName = std::wstring(L"BarnacleDev-") + ToHexWString(CertThumbPrint(newDevCert)) + std::wstring(L".cer");
        ...
        dfuFileName = dfuFileName.substr(0, dfuFileName.size() - 4) + std::string("-") + ToHexString(CertThumbPrint(newDevCert)) + std::string(".dfu");
